...
export default function V3({
  ...
  return (
    <div className={`${s.rows} ${gapCls(options.gap)} ${options.rounded ? s.rounded : ""}`}>
      {images.map((src, i) => (
        <figure key={i} className={s.rowItem}>
          ...
          {/* כשאין תמונה - הצג שתי אפשרויות בפנים */}
          {!src && (
            <div className={`${s.actions} ${s.actionsAlways}`}>
              <input
                id={`file-input-${i}`}
                type="file"
                accept="image/*"
              ...
              />
              <button
                className={`${s.actBtn} ${s.uploadBtn}`}
                type="button"
                onClick={() =>
