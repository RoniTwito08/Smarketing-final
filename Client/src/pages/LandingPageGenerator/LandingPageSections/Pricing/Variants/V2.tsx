// src/components/LandingPageSections/Pricing/Variants/V2.tsx
"use client";
...
export default function V2({
  ...
  return (
    <div className={`${s.grid} ${densityCls(options.density)} ${columnsCls(options.columns)} ${options.equalHeights ? s.equalHeights : ""}`}>
      {plans.map((p, i) => (
        <article key={i} className={`${s.card} ${s.soft} ${p.highlight ? s.highlight : ""}`}>
          {p.highlight && <div className={s.ribbon}>הבחירה הנפוצה</div>}
          ...
          <div className={s.cta}>
            ...
            <a className={`${s.btn} ${s.btnGhost}`} href={p.ctaHref || "#"}>למידע</a>
          </div>
        </article>
      ))}
    </div>
