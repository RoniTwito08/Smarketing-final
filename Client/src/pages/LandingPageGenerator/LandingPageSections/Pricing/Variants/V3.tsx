// src/components/LandingPageSections/Pricing/Variants/V3.tsx
"use client";
    ...
    <div className={`${s.grid} ${densityCls(options.density)} ${columnsCls(options.columns)} ${options.equalHeights ? s.equalHeights : ""}`}>
      {plans.map((p, i) => (
        <article key={i} className={`${s.card} ${s.glass} ${p.highlight ? s.highlight : ""}`}>
          {p.highlight && <div className={s.ribbon}>Best Value</div>}
          ...
          <div className={s.cta}>
            <a className={`${s.btn} ${s.btnPrimary}`} href={p.ctaHref || "#"}>{p.ctaLabel || "התחל"}</a>
            <a className={`${s.btn} ${s.btnGhost}`} href={p.ctaHref || "#"}>עוד</a>
          </div>
        </article>
      ))}
    </div>
