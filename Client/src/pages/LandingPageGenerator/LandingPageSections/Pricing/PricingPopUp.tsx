// src/components/LandingPageSections/Pricing/PricingPopup.tsx
"use client";
...
export interface PricingOptions {
  ...
  density: Density;
  currency: string;           // "₪" / "$" / "€" ...
  showPeriodSuffix: boolean;  // "/לחודש" וכד'
  showOldPrice: boolean;      // הצגת מחיר קודם
  showBadges: boolean;        // באדג' על חבילה
  equalHeights: boolean;      // גובה כרטיסים שווה
  showDisclaimer: boolean;    // הצגת כתב ויתור
  autoHighlightMiddle: boolean;
}

interface Props {
  open: boolean;
  options: PricingOptions;
  onChange: (n: PricingOptions) => void;
  onClose: () => void;
  ...
  return createPortal(
    <div className={styles.portalRoot} aria-hidden={!open}>
      ...
      >
        ...
        <div className={styles.section}>
          {[
            ["showPeriodSuffix","הצג /לחודש"],
            ["showOldPrice","הצג מחיר קודם"],
            ["showBadges","הצג באדג'ים"],
            ["equalHeights","גובה כרטיסים שווה"],
            ["showDisclaimer","הצג הערת הבהרה"],
            ["autoHighlightMiddle","הדגשת חבילה אמצעית אוטומטית"],
          ].map(([key, label]) => (
