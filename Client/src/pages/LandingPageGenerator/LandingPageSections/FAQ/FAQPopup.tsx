// src/components/LandingPageSections/FAQ/FAQPopup.tsx
"use client";
import { useEffect, useLayoutEffect, useRef, useState } from "react";
import { createPortal } from "react-dom";
import styles from "../Hero/LayoutPopUp/LayoutPopUp.module.css";

type Columns = "auto" | "single" | "double";
type Density = "compact" | "normal" | "spacious";
type ExpandMode = "details" | "accordion";
...
export default function FAQPopup({
  open, options, onChange, onClose, anchorRef, dir = "rtl",
}: Props) {
  ...
  useEffect(() => { if (open) onChange(local); }, [local, open, onChange]);
  ...
  useLayoutEffect(() => {
    if (!open) return;
    const place = () => {
      const a = anchorRef.current; const b = popRef.current; if (!a || !b) return;
      const ar = a.getBoundingClientRect(); const bw = b.offsetWidth || 380; const bh = b.offsetHeight || 300; const gap = 8;
      let left = ar.left + ar.width / 2 - bw / 2; let top = ar.bottom + gap;
  ...
  return createPortal(
    <div className={styles.portalRoot} aria-hidden={!open}>
      ...
      >
        ...
        <div className={styles.section}>
          <div className={styles.rowBetween}>
            ...
            <div className={styles.segment}>
              {(["auto","single","double"] as const).map(k => (
                <button key={k} className={`${styles.segmentBtn} ${local.columns === k ? styles.active : ""}`}
                        onClick={() => setLocal(p => ({ ...p, columns: k }))}>
                  {k === "auto" ? "Auto" : k === "single" ? "1" : "2"}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className={styles.rowBetween}>
            ...
            <div className={styles.segment}>
              {(["compact","normal","spacious"] as const).map(k => (
                <button key={k} className={`${styles.segmentBtn} ${local.density === k ? styles.active : ""}`}
        ...
        <div className={styles.section}>
          <div className={styles.rowBetween}>
            <span className={styles.label}>מצב פתיחה</span>
            <div className={styles.segment}>
              {(["details","accordion"] as const).map(k => (
                <button key={k} className={`${styles.segmentBtn} ${local.expandMode === k ? styles.active : ""}`}
                        onClick={() => setLocal(p => ({ ...p, expandMode: k }))}>
                  {k === "details" ? "Details" : "Accordion"}
                </button>
              ))}
            </div>
          </div>
          ...
          <label className={styles.rowCheck}>
            <input type="checkbox" checked={local.showNumbers}
                   onChange={e => setLocal(p => ({ ...p, showNumbers: e.target.checked }))}/>
            <span>מספור שאלות</span>
          </label>

          <label className={styles.rowCheck}>
            <input type="checkbox" checked={local.equalHeights}
                   onChange={e => setLocal(p => ({ ...p, equalHeights: e.target.checked }))}/>
            <span>גובה קלפים שווה</span>
          </label>
        </div>
